...
- **Movie** — hero shows exactly one primary state (D19):
  - **no copy** → primary ▶ **Download** (friendly S3 movie source picked per the D22 rule: default most-seeded, or best-seeded **web-exhibitable** source in `Web-playable first` mode with most-seeded fallback when nothing qualifies) with an inline `Best match · quality · size · seeds` hint under the button (lead word `Web-playable` when D22 `web-playable` mode picked a web-exhibitable release); **Advanced** (`SourcePickerModal`); while the source fetch runs the CTA reads `Finding the best release…` (shimmer, never a dead button). The S3 fetch runs **only when no copy is owned yet** and is cached per title+audio (S3 api cache), so revisiting an owned title never re-asks Prowlarr; deleting the last copy lazily re-fetches and returns the hero to the offer state.
