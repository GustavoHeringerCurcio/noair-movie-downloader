...
| D18 | **Hover-trailer previews (Netflix-style)**: every `TitleCard` optionally plays the title's best trailer when hovered. Trailer metadata comes from TMDB `GET /movie/{id}/videos` / `GET /tv/{id}/videos` through the existing key — **no new key, no YouTube Data API, no backend storage** (`S15`). On a sustained hover the card overlays the muted, looping official embed (`youtube-nocookie.com`, Vimeo fallback) over the still art; nothing is served from this box. TV cards play the **series** trailer (per-episode trailers are not surfaced — TMDB coverage is sparse). A title with no usable video keeps static art; any fetch failure degrades to "no trailer", never an error state. Embedding requires the *browser* to reach YouTube/Vimeo; the LAN box only needs TMDB reachability. |
