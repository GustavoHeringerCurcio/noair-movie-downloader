...
| D15 | Frontend visual language: **Netflix-style, strict black & white**. Left sidebar removed; fixed top header transparent at top that fades to a black gradient on scroll. Home = static cinematic brand hero (user-supplied looping GIF, grayscaled) + landscape 16:9 title-card rails that bleed full-width with the rightmost card clipped to invite scroll; full-screen search overlay. Rail named "My Downloads"; **local-only playback resume** (C-DL2); **every friendly download opens a confirm sheet** naming the exact pick (C-DL1); **inline first-run guidance** (C-DL3). |
