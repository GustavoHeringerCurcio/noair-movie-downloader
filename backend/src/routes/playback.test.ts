...
vi.mock('../lib/packages.js', () => {
  const fake: { phase: string; progress: number; error: string | null; master: string | null } = {
    phase: 'packaging',
    progress: 0,
    error: null,
    master: null,
...
beforeEach(() => {
  __setFake({ phase: 'packaging', progress: 0, error: null, master: null });
});
